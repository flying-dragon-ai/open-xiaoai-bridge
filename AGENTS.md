...
2. `killall tts_play.sh miplayer` — 停止阻塞 TTS
3. `mphelper pause` — 尝试停止非阻塞 TTS
4. `stop_playing` + `start_playing` — 重置 PCM 通道
5. `start_recording` — 恢复录音（KWS 依赖此通道）
6. `XiaoAI.stop_conversation()` — 停止连续对话
